	...
	class CR_Ajax_Reviews
	{
		...
		public static function get_reviews( $product_id ) {
			...
			foreach ($reviews as $review) {
				...
				$replies = array_merge( $replies, $replies_temp );
			}
				...
				if( isset( $_POST['rating'] ) ) {
					if( 0 <= $_POST['rating'] && 6 > $_POST['rating'] ) {
						...
						if( 0 < self::$rating ) {
							...
							$filter_note = '<div id="cr-ajax-reviews-fil-sta"><span>' . $filtered_comments . '</span><a class="cr-seeAllReviews" data-rating="0" href="' . esc_url( get_permalink( $_POST['productID'] ) ) . '#tab-reviews">' . $all_comments . '</a></div>';
						}
					}
				}
			}
