...
class Ivole_Reminders_List_Table extends WP_List_Table {
	...
	public function no_items() {
		if( 'cr' === get_option( 'ivole_scheduler_type' ) ) {
			/* translators: please keep '%1s' and '%2s' as is   */
			echo sprintf( __( 'The plugin is configured to use CR Cron for scheduling review reminders (%1s\'Reminders Scheduler\' setting%2s).', 'customer-reviews-woocommerce' ), '<a href="' . admin_url( 'admin.php?page=ivole-reviews-settings' ) . '" title="Plugin Settings">', '</a>' );
			echo ' ';
			/* translators: please keep '%1s' and '%2s' as is   */
			echo sprintf( __( 'Please log in to your account on %1sCusRev website%2s to view and manage the reminders.', 'customer-reviews-woocommerce' ), '<a href="https://www.cusrev.com/login.html" target="_blank" rel="noopener noreferrer">', '</a>' );
		} else {
